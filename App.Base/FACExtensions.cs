...
namespace FAC.Base
{
    public static class FACExtensions
    {
        /// <summary>
        ...
        /// <returns></returns>
        public static DbCommand LoadStoredProc(this DbContext context, string storedProcName, bool prependDefaultSchema = true, short commandTimeout = 30, bool isCreateNewConnection = true)
        {
            var cmd = isCreateNewConnection ? new SqlConnection(context.Database.GetDbConnection().ConnectionString).CreateCommand() : context.Database.GetDbConnection().CreateCommand();
        ...
        public static T? ExecuteStoredProcValues<T>(this DbCommand command) where T : struct
        {
            using (command)
            {
                if (command.Connection.State == System.Data.ConnectionState.Closed)
                ...
                try
                {
                    ...
                    {
                        var sprocResults = new SprocResults(reader);
                        return sprocResults.ReadToValue<T>();
